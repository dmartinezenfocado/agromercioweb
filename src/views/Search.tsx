...
export const Search = () => {
	...
	// 		document.title = `${title} | Agromercio 📦`
							...
							<div className='search-main gap-8 flex w-full xl:w-5/6 max-w-6xl mx-auto mt-8'>
								...
								<section className='search-catalog bg-white shadow-sm rounded-sm w-full'>
									<ol className='flex flex-col'>
										{state.results.map((product: {id: string, title: string, thumbnail: string, thumbnail_id: string, shipping: {free_shipping: boolean}, price: number }) => (
											<li key={product.id}>
												<div className='product-card bg-white py-5 px-5 flex gap-5 '>
													<div className='card-img-container w-40 h-40 min-w-[160px]'>
														<img onClick={() => handleSelectProduct(product.id)} className='cursor-pointer object-contain' src={product.thumbnail}/>
													</div>
